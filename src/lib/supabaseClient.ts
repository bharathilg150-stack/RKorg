...
const rawSupabaseUrl = 
  (typeof window !== 'undefined' ? (importMeta.env?.NEXT_PUBLIC_SUPABASE_URL || importMeta.env?.VITE_SUPABASE_URL || importMeta.env?.SUPABASE_URL) : undefined) || 
...
const supabaseAnonKey = 
  (typeof window !== 'undefined' ? (
    importMeta.env?.NEXT_PUBLIC_SUPABASE_ANON_KEY || 
    importMeta.env?.NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY || 
    importMeta.env?.VITE_SUPABASE_ANON_KEY || 
    importMeta.env?.SUPABASE_ANON_KEY
  ) : undefined) || 
...
export function getSupabaseAdmin() {
  ...
  if (!supabaseAdminClient) {
    ...
    if (!serviceKey) {
      console.warn('SUPABASE_SERVICE_ROLE_KEY / SUPABASE_SECRET_KEY missing. Falling back to public key.');
    }

    supabaseAdminClient = createClient(supabaseUrl || 'https://placeholder-project-id.supabase.co', serviceKey || 'placeholder-key', {
      auth: {
        persistSession: false,
        autoRefreshToken: false,
      }
